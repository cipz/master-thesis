...
\chapter{Technologies}\label{chapter:technologies}
	...
	\section{Radio technologies}\label{sec:radio_tech}
		...
		\subsection{LoRa and LoRaWAN}\label{subsec:lora_lorawan}
			...
			Although LoRa shows good performance for long-range transmission in open areas, its signals ``\textit{can be attenuated over distance, and buildings, trees, and other radio signal sources may interfere with the signals}'' \cite{8326735}
	...
	\section{Hardware (Microcontrollers)}\label{sec:microcontrollers}
		...
		\begin{figure}[H]
			...
			\caption{Attiny 85 on the left, two boards based on the ESP32 in the middle, Asus Tinker Board 2 on the right}
